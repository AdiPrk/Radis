...
void RunImGuiApplication()
{
    ...
    // Create a window
    GLFWwindow* window = glfwCreateWindow(1280, 720, "Radis Launcher", nullptr, nullptr);
    if (!window) {
    ...
    while (!glfwWindowShouldClose(window))
    {
        ...
        ImGui::Begin("Radis Launcher", nullptr,
        ...
        ImGui::BeginChild("RightPanel", ImVec2(rightPanelWidth - 20, 0), true);
        {
            if (selectedProject != -1 && selectedProject < projectNames.size()) {
                ...
                ImGui::Text("Welcome to Radis Launcher");
                ImGui::Spacing();
...
void LaunchApplication(const std::string& projectName, std::map<std::string, HANDLE>& runningProcesses) 
{
    ...
    RadisLaunch::EngineSpec engineSpec;
    engineSpec.name = UTF8ToWString(projectName);
    engineSpec.width = 1280;
    engineSpec.height = 720;
    engineSpec.fps = 0;
    engineSpec.serverAddress = Radis::SERVER_IP;
    engineSpec.serverPort = 7777;
    engineSpec.graphicsAPI = Radis::GraphicsAPI::Vulkan;
    ...
    // Create the command line for Radis.exe.
    ...
    // Launch Radis.exe in a separate process, using the client directory as the working directory.
    if (CreateProcessW(
    ...
    {
        MessageBoxA(nullptr, "Failed to start Radis.exe! Make sure the Client directory and Radis.exe exist.", "Error", MB_OK | MB_ICONERROR);
    }
