...
namespace Radis
{
    ...
    void UnifiedMeshes::AddMesh(Device& device, Mesh& mesh)
    {
        ...
        // Append CPU data
        mUnifiedMesh.mVertices.insert(mUnifiedMesh.mVertices.end(),
            mesh.mVertices.begin(), mesh.mVertices.end());
        mUnifiedMesh.mIndices.insert(mUnifiedMesh.mIndices.end(),
            mesh.mIndices.begin(), mesh.mIndices.end());
